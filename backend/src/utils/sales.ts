...
async function getTypesenseApiKey(): Promise<string> {
  ...
  const apiKey =
    data?.data?.serviceConfig?.search?.universalSearch?.clientConfig?.apiKey;

  if (!apiKey) {
    throw new Error(
      "No Typesense API key found in SearchServiceConfig response",
    );
  }
  ...
  try {
    ...
    if (base64Match) {
      const parsed = JSON.parse(
        Buffer.from(base64Match[0], "base64").toString(),
      );
...
async function searchAltAsset(
  query: string,
): Promise<{ assetId: string; gradingCompany: string; grade: string } | null> {
...
async function getItemSales(query: string): Promise<any[]> {
  ...
  const sales = transactions.slice(0, 10).map((tx: any) => {
